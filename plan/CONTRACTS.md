...
## Header Propagation Audit (#62)

Ratified 2026-04-23 by gateway-cleanup-expert. Scope: determine whether downstream services read `x-tenant-id` / `x-tenant-slug` from HTTP headers or gRPC metadata vs. expecting tenant context as a proto field on the RPC request. Output gates #64 (what the api-gateway must propagate once `tenant_id` is available in `AuthContext`).

### Gateway propagation today (HEAD: `c16d33f`)

- **HTTP proxy path** (`handlers/gateway/proxy.rs`): forwards every non-hop-by-hop request header verbatim. `X-Tenant-Id`, `X-Tenant-Slug`, `X-User-Id`, `Authorization`, `traceparent`, `tracestate` all pass through. Used by `/api/tenants/:id/{plan,access,usage}` (#54), `/api/onboarding/:id/*` (#37), `/api/auth/sso/*` + `/api/auth/impersonate*` (#36).
- **gRPC transcode path** (`grpc/dynamic_client/metadata.rs::inject_trace_headers`): only injects what the caller hands in via the `trace_headers` HashMap. `handlers/gateway/backend.rs::extract_trace_context` currently populates that map from the active OTEL span via the global text-map propagator — so `traceparent` + `tracestate` land on the outgoing gRPC metadata. **`X-Tenant-Id` / `X-Tenant-Slug` / `X-User-Id` do NOT currently propagate through the gRPC path.**

### Service consumption matrix

| Service                               | Header read from HTTP / gRPC md                                                                                                                                                                                 | Proto field on request                                                                                                               | Notes                                                                                                                                                                                                                        |
| ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **course-service** (NestJS)           | **YES** — `X-Tenant-Id` header in `auth/decorators/tenant.decorator.ts` + controllers (`analytics`, `lectures`, `course-rest`); `X-Tenant-Slug` fallback to `X-Tenant-Id` in `trace-propagation.interceptor.ts` | No tenant field on most REST requests (tenant comes from header or JWT)                                                              | Tenant **required** via header when JWT tenantId absent. Currently can't fail loudly because gateway passes it through the HTTP proxy path, but course-service's gRPC surface would need gRPC-metadata propagation to match. |
| **metrics-service** (Go)              | **YES** — `X-Tenant-Id` / `X-Tenant-Slug` read in 5+ handlers (`handlers.go`, `roster_handler.go`, `tracing_middleware.go`)                                                                                     | No tenant field on REST query                                                                                                        | Required for student-progress + course-metrics paths; falls back to DB lookup if header absent. All traffic is HTTP-proxied today; no gRPC route exercises this path.                                                        |
| **email-service** (Go)                | **YES** — `X-Tenant-Id` in `handlers/handlers.go:35` + full test coverage                                                                                                                                       | Proto has `tenant_id` field (`proto/email.proto:19,33,82`)                                                                           | Dual-mode: HTTP handler reads header, gRPC server reads proto field. Either source works.                                                                                                                                    |
| **content-management-service** (Rust) | **YES** — `X-Tenant-Id` required in `content_handlers.rs:108` (returns 400 if missing)                                                                                                                          | No tenant field in `content.proto` request messages                                                                                  | Header is the ONLY source of truth; gRPC transcode path would break without propagation.                                                                                                                                     |
| **incident-service** (Go)             | **YES** — `X-Tenant-Slug` in `cmd/server/main.go:214`                                                                                                                                                           | `tenant_id` optional on 3 messages in `incident.proto`                                                                               | Header used by HTTP entry; gRPC messages carry tenant_id explicitly when needed.                                                                                                                                             |
| **discussion-service** (Go)           | **YES** for tracing — pulls `x-tenant-slug` off gRPC metadata in `internal/grpc/server.go:121` + `propagation_test.go`                                                                                          | Required `tenant_id` on `GetInboxRequest`, `CreateThreadRequest`, `CreatePostRequest`, etc. (not optional — service 400s when empty) | **Tenant context MUST come from proto field.** Header read is only for trace-tagging, not request routing/authorization.                                                                                                     |
| **scheduling-service** (Go)           | **YES** for tracing — `x-tenant-slug` pulled off gRPC md in `internal/grpc/correlation.go`                                                                                                                      | Proto carries tenant via explicit fields in request messages                                                                         | Same pattern as discussion — header is tracing; proto field is authoritative.                                                                                                                                                |
| **ai-service** (Go)                   | **YES** for tracing — `x-tenant-slug` pulled off gRPC md in `internal/grpc/interceptor.go`                                                                                                                      | Proto fields on most requests                                                                                                        | Header is tracing-only; claude.go attaches it to Kafka headers for downstream span continuity.                                                                                                                               |
| **video-conferencing-service** (Rust) | **YES** for tracing + Kafka — `x-tenant-slug` read in `src/grpc/service.rs` + `src/kafka/consumer.rs`                                                                                                           | `tenant_slug` is a **required proto field** on 10+ request messages (`proto/video_conferencing.proto:63-141`)                        | Proto field is authoritative for request routing; header is tracing + Kafka span linkage only.                                                                                                                               |
| **feature-flag-service** (Go)         | Reference only (comment) — `x-tenant-slug` mentioned in `internal/grpc/server.go:121` but derived from correlation                                                                                              | `tenant_id`, `tenant_slug` as required proto fields on 4+ messages (`proto/feature_flag.proto:28,29,40,45`)                          | Proto field is the contract.                                                                                                                                                                                                 |
| **tenant-service** (Go)               | Injects `X-Tenant-Id` into Traefik-routed per-tenant traffic via `internal/traefik/generator.go` + `internal/docker/provisioner.go`. Reads it on its own handlers indirectly.                                   | `tenant_id` in `proto/tenant.proto` where relevant                                                                                   | This service is the _source_ of the header downstream — it doesn't itself consume gateway-provided `X-Tenant-Id`.                                                                                                            |
| **user-auth-service** (Go)            | No runtime read of `x-tenant-*`; impersonation JWT carries `tenant_id` in claims (`internal/auth/impersonation.go:58`)                                                                                          | `tenant_id` on `user.UserService` request messages (e.g. `ResolveUsername`, `proto/user.proto:260`)                                  | Auth/JWT is the primary tenant source. Normal login JWT does **not** currently carry `tenant_id` — see #61 follow-up.                                                                                                        |
| **api-gateway** (Rust)                | Self-reads `X-Tenant-Slug` for internal tenant routing (`handlers/gateway/routing.rs`), and forwards via HTTP proxy                                                                                             | N/A                                                                                                                                  | Auth middleware populates `AuthContext.tenant_id` from JWT claims once #64 lands.                                                                                                                                            |

### Buckets

- **Headers-only (no proto field)**: content-management-service, metrics-service (REST), course-service (REST controllers). These services **will break** if `X-Tenant-Id` stops arriving. Gateway HTTP proxy path already forwards it; gRPC transcode path does not (none of these services expose their tenant-read endpoints via gRPC today).
- **Proto-field-only (headers are tracing-only)**: discussion-service, scheduling-service, ai-service, feature-flag-service, video-conferencing-service (required `tenant_slug` field). These services ignore `x-tenant-*` for authorization — they depend on the gateway populating `tenant_id`/`tenant_slug` on the proto request message. That path is #61's Gap 2 injection → blocked on #64 filling `AuthContext.tenant_id`.
- **Dual-mode**: email-service, incident-service. Accept either source. Safe under both propagation gaps.

### Implications for #64 (gateway tenant-id derivation)

1. Once `AuthContext.tenant_id` is populated (from JWT claims **or** `Host` subdomain per #64 scope), the `add_auth_context()` injection (#61) automatically lands it on every gRPC request body as `tenant_id` / `tenant_slug`. **That is sufficient for the proto-field-only services.**
2. For headers-only services (currently all HTTP-path), the HTTP proxy already forwards `X-Tenant-Id` if the client sent it. **But if the gateway derives tenant from JWT or Host and the client didn't send the header, nothing is added today.** Fix: in the HTTP proxy path, after deriving `AuthContext.tenant_id`, add `X-Tenant-Id: <id>` to the outgoing request if not already present.
3. The gRPC metadata path does **not** need to propagate `x-tenant-slug` today (only tracing-only services read it from metadata; they read `traceparent` which is already propagated). **Skip adding to `inject_trace_headers` until a service actually requires it for authorization.**

### Action items banked for #64

- #64-a (gateway): derive tenant_id (JWT subclaim or Host subdomain), populate `AuthContext.tenant_id`.
- #64-b (gateway): inject `tenant_id` / `tenant_slug` onto proto body in `add_auth_context` — **already done by #61** once AuthContext has the value.
- #64-c (gateway): inject `X-Tenant-Id` header on HTTP-proxy outbound if `AuthContext.tenant_id.is_some()` and client didn't already send it.
- #64-d (gateway): OPTIONAL — add `x-tenant-slug` to gRPC metadata alongside traceparent. Only needed if a future service requires metadata-based authorization. Skip for now.

HEAD at audit time: `c16d33f`. No service handler changes required to unblock #64 — the audit confirms gateway-side injection alone is sufficient, per bucket-2 services' reliance on proto fields.
