...
**Resolution (§1b, CLOSED `22b4d70` + cleanup `2ebfa8c`):** 14 Restate routes added to gateway-config.yaml using `http_proxy_target: http://restate-server:9080` (Restate ingress) + `http_proxy_path_template` for `:id` substitution (matcher key is `:id`, matching the rest of the yaml's convention; CanvasMigration nested under `/api/onboarding/:id/canvas/*`). Target is the Restate ingress port, NOT `onboarding-service:9080` (SDK endpoint is not publicly routable). Stale `onboarding.OnboardingService/{StartOnboarding,GetOnboardingStatus,CanvasImport}` gRPC passthroughs removed in `2ebfa8c`.

**Health-check plumbing (ratified 2026-04-22):** `/api/health/onboarding` is **not** a gRPC route and was never a candidate for a gRPC health shim. On HEAD (gateway-config.yaml:1143-1146) it is an HTTP-proxy route to `http://onboarding-service:9081` — the dedicated HTTP health sidecar `onboarding-service` spawns alongside its Restate :9080 handler (services/onboarding-service/src/main.rs:31-39). Prometheus scrape + Docker HEALTHCHECK target the sidecar directly; exposing it through the gateway is just a routing convenience. **P6 onboarding-health: dismissed from incident-expert's queue.** No gRPC facade on onboarding-service — that would fight the Restate framework. The original P6-onboarding-health dispatch was self-contradictory given po-analyst's §1a ruling; retroactively ratified as closed via option (a) + "no gRPC surface needed".
